...
- State pages: `/states/[stateCode]` show parsed state law data (if present) from markdown files in `dashboard/content/`.

State law data pipeline:
- `npm run dev` / `npm run build` runs `dashboard/scripts/generate-state-laws.mjs` to generate `dashboard/lib/state_laws.generated.json` from the markdown files in `dashboard/content/`.
